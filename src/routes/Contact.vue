      ...
      <section class="sec-simple">
        ...
        <div class="sec-simple__content">
          ...
          <form v-if="response === ''" class="contact-form" v-on:submit.prevent>
            ...
            <p><button v-on:click="contactSubmit" :disabled="sending">Submit &nbsp; <em class="fa fa-arrow-circle-o-right"></em></button></p>

            <input type="text" name="hp" id="hp" ref="honeypot" class="hp" value="" />
          </form>
          <ul>
            <li><a href="static/media/CaitlinMesiano.pdf" target="_blank"><span class="fa fa-file-pdf-o"></span> PDF copy of Caitlin's resume</a></li>
            <li><a href="https://www.linkedin.com/in/caitlin-mesiano-b5a54b108/" target="_blank"><span class="fa fa-linkedin-square"></span> LinkedIn</a></li>
          </ul>
        </div>
      </section>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      contactSubmit() {
        const errorText = 'Ooops! Something went wrong. Please try again. If the issue persists, please consider contacting Caitlin directly. Phone number and email address can be found in Caitlin\'s resume, below.<br><br>Thank you! And, sorry for any inconvenience.';
        ...
        if (!fullName || !email || (email && !emailValid) || !message) {
          ...
          axios.get('static/inc/contact_AJAX.php', {
            params: {
              email,
              name: fullName,
            ...
            .then((re) => {
              if (re.data === 1) {
                this.response = 'Your response has been sent! Thank you so much for contacting Caitlin. You will receive a response as soon as time permits.';
              } else {
                this.response = errorText;
              }
            })
